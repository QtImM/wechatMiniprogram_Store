...
## 2026-08-30 小程序下单可执行引导

- 已修复结算页无地址状态：无论接口返回空对象还是空地址，页面都会稳定显示“请先添加收货地址”的可点击卡片。
- 首次进入无地址结算页，以及无地址点击“提交订单”时，均会弹出“下单前需要添加收货地址”的确认引导；点击“去添加”会直接进入新增收货地址页，而不是只显示无法执行的 toast。
- 购物车结算和商品立即购买共用该结算页，因此两条下单路径均已覆盖；新增地址保存后返回结算页会重新读取并选中地址。
- 验证通过：`git diff --check` 通过；已确认 HBuilderX 和微信开发者工具均在运行。小程序仍应以 HBuilderX 内置编译器重新编译，避免 Node.js 24 与 uni-app Vue2 CLI 的已知兼容问题。

## 2026-08-30 小程序提示可执行性盘点

- 已盘点小程序页面和组件中约 60 处 toast、modal 与提示调用，并形成 `docs/acceptance/2026-08-30-miniapp-actionable-guidance-audit.md`。
- 已确认真正需要跨页面引导的高优先级场景是“下单缺收货地址”，本轮已完成；未登录、支付未完成、删除确认、订单操作和售后操作已有可执行入口。
- 商品失效清理、下单失败按后端原因分流、规格未选时自动突出选择区、无物流时按订单状态说明，列为下一批体验增强项；表单格式错误仍应在当前页定位，不宜强制跳转。

## 2026-08-30 小程序提示引导闭环

- 购物车失效商品现提供“一键清理失效商品”；勾选、全选、加数量和结算时都会给出明确处理入口，且全选状态只计算可结算商品。
- 下单失败会保留后端具体原因并按类型引导：缺地址去新增地址，商品/库存变动返回商品或购物车，优惠券失效重新选择，其余情况可刷新订单信息后重试。
- 商品规格未选完整时会自动打开规格选择区并高亮未选的规格组；订单列表和订单详情中的无物流提示改为备货中、物流同步中等可理解状态。
- 验证通过：`git diff --check` 通过；HBuilderX 内置编译器已自动更新购物车、商品、结算、订单和订单详情的 `mp-weixin` 构建产物，关键引导文案可在编译后的 JS 中检索到；仍建议在微信开发者工具中完成交互复验。

## 2026-08-30 可视化装修小白体验审查

- 已以首次使用管理后台的客户视角审查“可视化装修 / 实时预览与编辑”，结论归档至 `docs/acceptance/2026-08-30-visual-editor-novice-ux-audit.md`。
- 当前已有点击编辑、草稿预览、结构化控件、保存校验和一键回退；下一轮应优先补齐固定修改摘要、保存前可视化体检、可编辑地图与完整内容入口。
- 该审查只形成体验改进清单，未改变现有管理后台功能或接口。

## 2026-08-30 可视化装修小白增强

- 可视化装修新增固定草稿操作条：会清楚列出已修改字段，并提供“放弃本次修改”和“检查并发布”，避免小白忘记当前是否仍在草稿状态。
- 发布前改为先展示变更摘要和体检结果：必填、价格、库存、重复内容等问题会阻止发布；图片比例、上架零库存、跳转目标等会以可理解的提醒呈现。
- 首页预览各区块均可“查看全部”，可从完整的 Banner、频道、品牌、专题、商品列表中选择任一项回到预览编辑；不再把预览中的前几条误认为全部数据。
- Banner、频道的跳转目标会显示为商品名、专题名或固定页面名，并可在预览中测试商品、专题和首页跳转。
- 默认隐藏排序权重等低频字段，运营人员需点“更多设置”才会展开；保存成功后会明确提示可通过既有一键回退恢复最近操作。
- 验证通过：管理后台 `corepack pnpm typecheck`、`corepack pnpm build` 均通过。

## 2026-08-30 内容素材校验兼容

- 已修复 Banner、频道、品牌、专题在可视化装修中更换素材后被“必须使用 HTTPS 或小程序静态资源路径”误拦截的问题。
- 根因是内容管理仍使用旧的地址前缀校验，而素材库本地开发环境会返回 `http://127.0.0.1:8085/uploads/material/...`；内容管理现与商品管理统一复用素材库校验，素材库图片、配置白名单和既有 HTTPS 图片均可正常保存。
- `file://`、临时文件地址和非素材库 HTTP 地址仍会被拒绝，避免不安全或失效图片进入正式数据。
- 验证通过：商品模块 Maven 构建通过；Docker 后端镜像已重建并替换运行容器，`/actuator/health` 返回 `UP`。
